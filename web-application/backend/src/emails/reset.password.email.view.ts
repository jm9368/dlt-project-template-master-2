...
export class ResetPasswordEmailView {
    public static getSubject(request: Express.Request, user: User): string {
        ...
        return PLATFORM_NAME + " - " + translator.__("Password Reset");
    }
    ...
    public static render(request: Express.Request, user: User, urlToRedirect: string): string {
        ...
        html += `       <title>${PLATFORM_NAME} - ${translator.__("Reset your password")}</title>`;
        html += `   </head>`;
        html += `   <body>`;
        html += `       <h2>${PLATFORM_NAME} - ${translator.__("Reset your password")}</h2>`;
        html += `       <p>${translator.__("Hi {0}, in order to reset your password, follow the link below:").replace("{0}", escapeHTML(user.username))}</p>`;
    ...
    public static renderText(request: Express.Request, user: User, urlToRedirect: string): string {
        ...
        text += `${PLATFORM_NAME} - ${translator.__("Reset your password")}` + `\n`;
        text += `` + `\n`;
        text += `${translator.__("Hi {0}, in order to reset your password, follow the link below:").replace("{0}", escapeHTML(user.username))}` + `\n`;
